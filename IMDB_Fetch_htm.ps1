...
#                <#
                ...
                $returnObject = New-Object -TypeName HTML
                $returnObject = New-Object -TypeName PSObject -Property $WebPage

                #write-host $returnObject

                Write-Output $returnObject
 
                Remove-Variable IMDBItem, ItemInfo, ItemTitle, Genres, Description, Released, Type, Rating, Runtime, MPAARating, Stars, S, SS, WebPage -ErrorAction SilentlyContinue
            } catch {}
        }
        #write-Host "Exiting loop"
    }
    ...
    If ( $F.length -ge 5 ) { $X = $F.substring($F.length-4,4) }
        ...
        If ( ($X -eq ".mp4") -or ($X -eq ".mkv") -or ($X -eq ".avi") -or ($X -eq ".mpg") -or ($X -eq ".m4v") ) {
            ...
            If ( -not (Test-Path $Out -PathType Leaf) ) {
                ...
                (get-IMDBMatch -Title $Title | Where { $_.Released -eq $Year } | Where-Object { $_.Type -eq 'Movie'} | Get-IMDBItemHTML ) > $Out
                #write-host -NoNewline
                #write-host "Processed $Year $Title"

                <#
                # Changing "-eq 0" to "-lt 500" so under 500 is deleted
                If ( ( (Get-Item $OutHTML).length) -lt 500 ) {
